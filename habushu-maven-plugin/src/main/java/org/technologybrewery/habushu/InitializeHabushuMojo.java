...
public class InitializeHabushuMojo extends AbstractHabushuMojo {
    ...
    public void doExecute() throws MojoExecutionException, MojoFailureException {
      ...
      try {
          packageManagerCommandHelper.execute(Arrays.asList("check"));
      } catch (HabushuException e) {
          getLog().debug("Failure encountered while running 'poetry check'!", e);
          getLog().warn("Package Manager check failed (debug contains more details) - this is likely due to a "
                  + "mismatch between your pyproject.toml and poetry.lock file - attempting to correct...");
          packageManagerCommandHelper.execute(packageManagerCommandHelper.createLockCommand());
          getLog().warn("Corrected - pyproject.toml now synced");
      }

      String currentPythonPackageVersion = packageManagerCommandHelper.execute(Arrays.asList("version", "-s"));
      ...
      if (!StringUtils.equals(currentPythonPackageVersion, expectedPythonPackageVersion)) {
          if (overridePackageVersion) {
              getLog().info(String.format("Setting Package Manager package version to %s", expectedPythonPackageVersion));
              getLog().info(
              ...
              packageManagerCommandHelper.executeAndLogOutput(Arrays.asList("version", expectedPythonPackageVersion));
          } else {
              getLog().debug(String.format(
                      "Package Manager package version set to %s in pyproject.toml does not align with expected POM-derived version of %s",
