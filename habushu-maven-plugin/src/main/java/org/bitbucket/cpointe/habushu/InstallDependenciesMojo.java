...
public class InstallDependenciesMojo extends AbstractHabushuMojo {
	...
	protected Integer executePoetryCommandWithTimeout(PoetryCommandHelper poetryHelper, List<String> commandArgs,
			int timeout, TimeUnit timeUnit) {
		ExecutorService executor = Executors.newSingleThreadExecutor();
		Future<Integer> future = executor.submit(() -> poetryHelper.executeAndLogOutput(commandArgs));
		try {
			...
			throw new RuntimeException(String.format("Error occurred while performing Poetry command: poetry %s",
					StringUtils.join(commandArgs, " ")), e);
		} finally {
			executor.shutdown();
		}
	}
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (!this.skipPoetryLockUpdate) {
			...
			executePoetryCommandWithTimeout(poetryHelper, Arrays.asList("lock"), 2, TimeUnit.MINUTES);
		}
		...
		executePoetryCommandWithTimeout(poetryHelper, installCommand, 2, TimeUnit.MINUTES);
	}
