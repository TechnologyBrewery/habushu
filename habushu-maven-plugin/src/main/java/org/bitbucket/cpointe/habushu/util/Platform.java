...
/**
 * Pulled over from the the frontend-maven-plugin. We will not be making changes since it is tried and true.
 * 
...
public class Platform {
    ...
    public static Platform guess() {
        ...
        if (os == OS.Linux && new File("/etc/alpine-release").exists()) {
            return new Platform(
                    ...
                    "https://unofficial-builds.nodejs.org/download/release/", os, architecture, "musl");
        }
    ...
    public String getNodeDownloadFilename(String nodeVersion, boolean archiveOnWindows) {
        if (isWindows() && !archiveOnWindows) {
            ...
            return nodeVersion + "/" + getLongNodeFilename(nodeVersion, archiveOnWindows) + "."
                    + os.getArchiveExtension();
        }
    }

    public String getNodeClassifier() {
        final String result;
        if (isMac() && architecture == Architecture.arm64) { // this check is required to download the x64 binary until
                                                             // there is an arm64 version available for macOS (darwin).
