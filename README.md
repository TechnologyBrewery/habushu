...
used in Habushu's test modules to create a dependency such that `habushu-poetry-package-consumer` depends on `habushu_poetry_package`.
By adding this, any time a change occurs in `habushu_poetry_package`, all dependent Habushu modules will be rebuilt automatically.
...
Enables the automatic injection of containerization logic to the dockerfile for the `containerize-dependencies` goal.
...
- [Adding Habushu to an Existing Poetry Project](examples/poetry/habushu-poetry-add-to-project/README.md) - Adding Habushu to an existing Poetry project
- [Adding Habushu to an Existing uv Project](examples/uv/habushu-uv-add-to-project/README.md) - Adding Habushu to an existing uv project
...
add in Habushu dependencies in your project's `pom.xml`. You can find an example in [`habushu-poetry-package-consumer/pom.xml`
where it references a dependency to `habushu-poetry-package`](https://github.com/TechnologyBrewery/habushu/blob/dev/habushu-poetry-package-consumer/pom.xml)
...
been previously built or there are unbuilt changes to the `habusu` lifecycle, developers must manually build the 
`habushu-maven-plugin` and then execute **another, separate** build of `habushu-poetry-package` (and any other 
`habushu` module) to use the updated `habushu-maven-plugin` and `habushu` lifecycle.  Developers are **not** able to 
build updates to the `habushu` lifecycle and test their application in `habushu-poetry-package` within the same build.  
That said, if developers do not update the `habushu` lifecycle and simply make updates to existing `Mojo`s defined in 
